...
"let g:LookupFile_MinPatLength = 2               "最少输入2个字符才开始查>
...
let g:LookupFile_FileFilter = '\.class$\|\.o$\|\.obj$\|\.exe$\|\.jar$\|\.z
...
set smartcase    "如果搜索模式包含大写字符，不使用 'ignorecase' 选项，只有
...
set encoding=utf-8
" 自动判断编码时，依次尝试以下编码：
set fileencodings=ucs-bom,utf-8,cp936,latin-1,gbk,gb18030,big5,euc-jp,euc-kr,latin1
set helplang=cn
...
set fileencodings=ucs-bom,utf-8,cp936,latin-1,gbk,gb18030,big5,euc-jp,euc-kr,latin1     "设置支持打开的文件>

" 文件格式，默认 ffs=dos,unix
set fileformat=unix                                   "设置新（当前）文件>
set fileformats=unix,dos,mac                          "给出文件的<EOL>格式
...
"map <F2> :AddMinHeader<CR>     #Adds minified version of author informati
"Adding Lincenses
...
" // In order to avoid conflicts, the Source Explorer should know what plu
...
" // It only searches for a match with the keyword according to command 'g
let g:SrcExpl_searchLocalDef = 1
" // Set "<F6>" key for displaying the previous definition in the jump lis
